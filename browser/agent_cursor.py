...
# Standard Agent Theme Colors (Google Gemini Royal Purple/Blue, DeepSeek Blue, ChatGPT Emerald, Claude Ochre)
AGENT_CURSOR_COLORS: Dict[str, str] = {
    "atlas": "#2e6f54",
    "web_agent": "#06b6d4",    # Cyber Cyan / Full Screen Overseer
    "web-agent": "#06b6d4",
    "gemini": "#7c3aed",       # Royal Purple / Google Gemini
    "deepseek": "#2563eb",     # Deep Blue / Researcher
    "chatgpt": "#059669",      # Emerald / OpenAI
    "claude": "#d97706",       # Warm Ochre / Anthropic
    ...
    "dalle": "#ec4899",        # Magenta Pink / Creative
    "grok": "#e11d48",         # Ruby Red / Grok
}
...
(function() {
    function ensureCursorStyles() {
        if (!document.getElementById('agentic-antigravity-cursor-style')) {
    ...
    function ensureCursorElement() {
        let wrapper = document.getElementById('agentic-cursor-wrapper');
        if (!wrapper) {
            wrapper = document.createElement('div');
            wrapper.id = 'agentic-cursor-wrapper';
            const initX = (window.__agentic_cursor && window.__agentic_cursor.x) ? window.__agentic_cursor.x : 60;
            const initY = (window.__agentic_cursor && window.__agentic_cursor.y) ? window.__agentic_cursor.y : 150;
            ...
            wrapper.innerHTML = `
                ...
                <div class="agentic-cursor-badge" id="agentic-cursor-badge">
                    ...
                    <span id="agentic-cursor-name">Agent • Specialist</span>
    ...
    if (!window.__agentic_cursor) {
        window.__agentic_cursor = {
            ...
            setPosition: function(x, y) {
                this.x = x;
                this.y = y;
                const wrapper = ensureCursorElement();
                if (wrapper) wrapper.style.transform = `translate(${x}px, ${y}px)`;
            },
            setAgent: function(name, color = '#2e6f54', role = 'Specialist') {
                this.agentName = name;
            ...
            show: function() {
                ...
                if (this.agentName) {
                    this.setAgent(this.agentName, this.agentColor, this.agentRole);
                }
                if (this.actionText) {
                    this.setAction(this.actionText);
                }
            },
            hide: function() {
            ...
            screenSweep: function() {
                ...
                const container = document.body || document.documentElement;
                if (container) container.appendChild(sweep);
...
async def inject_agent_cursor(page, agent_name: str = "Gemini", agent_color: Optional[str] = None, agent_role: str = "Specialist"):
    """
    Injects the Antigravity cursor script and sets up the active agent's name badge & theme color.
    Uses add_init_script so that cursor stays active through all URL/thread navigations.
    """
    if not page:
        return
    
    clean_id = agent_name.lower().replace(" ", "_")
    color = agent_color or AGENT_CURSOR_COLORS.get(clean_id, "#7c3aed" if clean_id == "gemini" else "#2e6f54")
    ...
    (() => {{
        ...
        if (window.__agentic_cursor) {{
            ...
            const curX = (window.__agentic_cursor.x && window.__agentic_cursor.x > 20) ? window.__agentic_cursor.x : 60;
            const curY = (window.__agentic_cursor.y && window.__agentic_cursor.y > 50) ? window.__agentic_cursor.y : 150;
            window.__agentic_cursor.setPosition(curX, curY);
    ...
    try:
        await page.add_init_script(setup_code)
    except Exception:
        pass

    try:
        await page.evaluate(setup_code)
    except Exception:
        pass
...
async def simulate_agent_type(page, selector: str, text: str, agent_name: str = "Gemini"):
    ...
    await inject_agent_cursor(page, agent_name, agent_role="Leader" if agent_name.lower() == "gemini" else "Specialist")
    ...
    try:
        if selector:
            ...
            if box:
                ...
                dim = await page.evaluate("() => ({w: window.innerWidth || 800, h: window.innerHeight || 600})")
                target_x = 80
                target_y = max(150, dim.get("h", 600) - 120)
    except Exception:
        pass
