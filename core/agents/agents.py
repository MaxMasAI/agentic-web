...
async def talk_to_agent(agent_id, page, prompt_text):
    ...
    input_box = None
    matched_sel = input_selectors[0] if input_selectors else "textarea"
    ...
    ag = agentlist.get_agent_by_id(agent_id)
    is_lead = ag.get("is_leader", False) if ag else (agent_id.lower() == "gemini")
    role_label = "Leader" if is_lead else "Specialist Worker"

    try:
        from browser.agent_cursor import inject_agent_cursor, simulate_agent_type, set_cursor_action
        await inject_agent_cursor(page, agent_name=agent_id, agent_role=role_label)
        await simulate_agent_type(page, matched_sel, formatted_prompt, agent_name=agent_id)
    except Exception:
        pass
    ...
    try:
        from browser.agent_cursor import inject_agent_cursor, set_cursor_action
        await inject_agent_cursor(page, agent_name=agent_id, agent_role=role_label)
        await set_cursor_action(page, "✓ Output Ready")
    except Exception:
        pass
